...
class LibriPartyDataset(Dataset):
    ...
    def set_attribute(self, dataset, augment = True, perturb = False, rir = False, noise = True):
...
if __name__ == '__main__':
    ...
    #     SNR =  dataset.compute_snr(data['mix'][0,0], data['source'][0,0,0])
    #     # if SNR < -10:
    #     #     sf.write(f'sample/mix_{i}.wav', data['mix'][0].transpose(1,0).numpy(), 16000)
    #     #     sf.write(f'sample/source_{i}.wav', data['source'][0,0].transpose(1,0).numpy(), 16000)
