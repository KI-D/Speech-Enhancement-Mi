...
class GlobalLayerNorm(nn.Module):
    ...
    '''
    def __init__(self, dim, last = False, time=False):
...
class SequenceModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bidirectional, timeasfeature = False, sequence_model="GRU",output_activate_function="Tanh"):
        """
        ---------------------------------------
        import torch
        ...
        if bidirectional:
            self.fc_output_layer = nn.Linear(hidden_size * 2, input_size)
        else:
            self.fc_output_layer = nn.Linear(hidden_size, input_size)

        self.norm = GlobalLayerNorm(input_size, last=True, time=False)
    ...
    def forward(self, x):
        ...
        x, h = self.sequence_model(x, self.h)
...
class GeneralBeamformer(nn.Module):
    """
    ---------------------------------------
    import torch
    from modules import GeneralBeamformer
    model = GeneralBeamformer([4,8,16,32,64], 201, 128, 256, 400)
    x = torch.rand(3, 3, 201, 6, 2)
    y = model(x)
    """
    def __init__(self, num_channels, num_freqs, hidden, segment_length, num_layers = 1, num_inputs=3, 
                kernel_size=3, dropout=0.0, sample_rate=16000,  win_length=25, hop_length=10, n_fft=400):
        super(GeneralBeamformer, self).__init__()
        self.segment_length = segment_length
        self.num_freqs = num_freqs
        self.num_time = segment_length // 160 + 1
        ...
        for i in range(num_levels):
            ...
            if i==0:
                deconvs = [TemporalConvTranspose2d(out_channels, 4 * num_inputs * 9, (5,kernel_size), stride=(2,1), dilation=(1,dilation_size),
                            padding=(2,(kernel_size-1) * dilation_size), dropout=dropout, activation=activation)]
            else:
        ...
        self.ln_S = GlobalLayerNorm(num_freqs*self.num_time)
        self.ln_N = GlobalLayerNorm(num_freqs*self.num_time)

        self.gru_S = SequenceModel(num_inputs*num_inputs, hidden, num_layers, 
                                    False, sequence_model="GRU", output_activate_function=activation)
        self.gru_N = SequenceModel(num_inputs*num_inputs, hidden, num_layers, 
                                    False, sequence_model="GRU", output_activate_function=activation)
        self.linear = nn.Sequential(nn.Linear(9, hidden), nn.ReLU(), GlobalLayerNorm(num_freqs), nn.Linear(hidden, 6))
    ...
    def forward(self, x):
        ...
        for m in self.deconvlist[:-1]:
            ...
            index -= 1
        #last one, [B, 4*M*9, F, T]
        x = self.deconvlist[-1](x).reshape(B, 2, 2, M, 9, F*T)
        del residuals

        #[B, M, 9, F*T] complex
        #x = decompress_cIRM(x)
        S = ComplexTensor(x[:,0,0], x[:,0,1])
        N = ComplexTensor(x[:,1,0], x[:,1,1])
        #[B, M, 9, F*T, 2]
        x = torch.nn.functional.unfold(noisy.reshape(B,M,F,T*2), (3,3), padding=1).reshape(B, M, 9, F*T, 2)
        x = ComplexTensor(x[...,0], x[...,1])
        #[B, F*T, M, 1] complex
        S = (S * x).sum(dim=2).transpose(1,2).unsqueeze(-1)
        N = (N * x).sum(dim=2).transpose(1,2).unsqueeze(-1)
        #[B, F*T, M, M] complex
        Phi_S = S @ S.conj().transpose(-1, -2)
        Phi_N = N @ N.conj().transpose(-1, -2)
        del S, N
        #[B, F*T, M, M]
        Phi_S = self.ln_S(Phi_S.real)
        Phi_N = self.ln_N(Phi_N.real)
        #[B, F*M*M, T]
        Phi_S = Phi_S.reshape(B, F, T, M, M).permute(0, 1, 3, 4, 2).reshape(B*F, M*M, T)
        Phi_N = Phi_N.reshape(B, F, T, M, M).permute(0, 1, 3, 4, 2).reshape(B*F, M*M, T)
        Phi_S = self.gru_S(Phi_S)
        Phi_N = self.gru_N(Phi_N)
        #[B, F, T, M*M]
        Phi = (Phi_S * Phi_N).reshape(B, F, M*M, T).transpose(2,3)
        del Phi_S, Phi_N
        #[B, F, T, M, 2]
        w = self.linear(Phi).reshape(B, F, T, M, 2).conj()
        del Phi
        #[B, M, F, T, 2] -> [B, F, T, M, 2]
        noisy = noisy.permute(0, 2, 3, 1, 4)
        real = w[..., 0] * noisy[..., 0] - w[..., 1] * noisy[..., 1]
        img = w[...,0] * noisy[...,1] + w[...,1] * noisy[...,0]
        #[B, F, T, 2]
        noisy = torch.sum(torch.stack([real, img], dim=-1), dim=-2)
        return noisy
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        model = GeneralBeamformer(num_channels=[128, 128, 256, 256], num_freqs=201, hidden=300, 
