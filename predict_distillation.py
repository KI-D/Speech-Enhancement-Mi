...
def predict(args):
    ...
    stage_dir = os.path.join('dillations/denoise',config['user_defined_name'])
    name = args.name
    ...
    model = globals()[name](**config[sub_name[name]]).cuda()
    path = os.path.join(stage_dir, name+'.pth')
    model.load_state_dict(torch.load(path), strict=False)
    print(sum([param.numel() for param in model.student.parameters()]) / 1e6)
    
    ##LSTM只能做静态量化
    model = model.cpu()
    #model = torch.quantization.quantize_dynamic(model, {torch.nn.GRU, torch.nn.Linear}, dtype=torch.qint8)
    # print(model)
    # torch.save(model.student.state_dict(), 'distillation.pth')
    ...
    for data in dataloader:
        ...
        with torch.no_grad():
            ...
            start = time.time()
            separated, _ = model.student.realtime_process(mixture, flag=False)
